...
validate_args() {
    ...
    # Validate username format (standard Linux username rules)
    if ! [[ "$USERNAME" =~ ^[a-z_][a-z0-9_-]{0,31}$ ]]; then
        die "Invalid username '${USERNAME}': must start with a lowercase letter or underscore, contain only [a-z0-9_-], max 32 chars"
    fi
...
generate_secrets() {
    ...
    if command_exists uuidgen; then
        ...
        if [[ -f /proc/sys/kernel/random/uuid ]]; then
            ...
            # Generate valid UUID v4: set version nibble (4) and variant bits (8-b)
            local hex
            hex=$(openssl rand -hex 16)
            UUID="${hex:0:8}-${hex:8:4}-4${hex:13:3}-$(printf '%x' $(( 0x${hex:16:1} & 0x3 | 0x8 )))${hex:17:3}-${hex:20:12}"
        fi
    fi
...
create_user() {
    ...
    if id "$USERNAME" &>/dev/null; then
        ...
        # Allow sudo for this user (password required)
        echo "${USERNAME} ALL=(ALL) ALL" > "/etc/sudoers.d/${USERNAME}"
        chmod 440 "/etc/sudoers.d/${USERNAME}"
        log_ok "User ${USERNAME} created with sudo access"
    fi

    # Setup SSH key (append if not already present)
...
SSHD_EOF
    ...
    # Verify config before restarting; restore backup on failure
    sshd -t || { cp /etc/ssh/sshd_config.bak /etc/ssh/sshd_config; die "SSH config validation failed! Backup restored."; }
...
setup_singbox_logs() {
    ...
    # Setup log rotation
    cat > /etc/logrotate.d/sing-box <<'LOGROTATE_EOF'
...
LOGROTATE_EOF
    ...
    log_ok "Log directory and rotation configured"
}
...
save_secrets() {
    ...
    local _generated
    _generated=$(date -u '+%Y-%m-%d %H:%M:%S UTC')
